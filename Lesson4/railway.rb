class Railway
  ...
  def create
    station1 = Station.new('station0')
    station2 = Station.new('station00')
    ...
    route = Route.new(station1, station2, 'route66')
    route.add(station2)

    @routes = [route]

    train = PassengerTrain.new('train0')
  ...
  private
  #Данные методы также должны вызываться только внутри класса
  def stations_list
  ...
  def carriage
    ...
    if train_exist?(number)
      ...
      when '1'
        ...
        carriage = Carriage.new('carriage', type)
        select_train(number).add_carriage(carriage)
      when '2'
        select_train(number).unhook_carriage
      else
        error_menu
      end
    else
